...
**--base:** The full tag (including "wigwam-") of the ISCE3 mamba development environment image generated by the setup-all step. This is the base image for the stack of images generated by the build-all step.  
...
Many of the optional build arguments are not necessary to complete the ISCE3 distributable build. For most typical use cases, the following minimal path is all that is necessary to acquire an ISCE3 distributable image with WIGWAM. To build ISCE3 from a repository stored on the local machine, the following steps are necessary:
    ...
    python -m wigwam setup all
    python -m wigwam build-all -b wigwam-setup-mamba-dev --copy-path {path/to/isce3_repo}
    python -m wigwam distrib -b wigwam-setup-mamba-runtime -s wigwam-build-installed
...
The build-all step can be modified in a small way to build from an online repository instead of a local one:
    ...
    python -m wigwam build-all -b wigwam-setup-mamba-dev --repo {user}/{repo_name}
...
The simple case for running a workflow requires only a handful of arguments. Assume that the user wants to run an `rslc` workflow test called `example_test`:
