...
The three-step process of generating the ISCE3 distributable is as follows: The setup-all step, the build-all step, and the distrib step. The setup-all step creates the stack of images which form the runtime and development environments for ISCE3 from some base image. The build-all step takes in the final "dev environment" image produced by the last step and acquires and installs the ISCE3 repository in a stack of images. Finally, the deployable step copies the installed ISCE3 framework from its image generated by the build-all step into the slimmer runtime environment image from the setup-all step to create a final, slim ISCE3 distributable image. These will be explained in further detail below.

For users who need to tailor a more specific build, this process is split into subcommands later in this document.

![](.docs/wigwam_flow.png)
...
The build-all step generates four images: Either a git repo image or a local ISCE3 repo image, followed by CMake configuration, ISCE3 build, and ISCE3 installed images. The repo images intake the ISCE3 codebase from either the head of a remote Git repository or from a local file, and the CMake images configure CMake, build ISCE3, and then install it to the "/app" folder on the image. All images in this step are built without the cache.

**--base:** The tag of the ISCE3 mamba development environment image generated by the setup-all step. This is the base image for the stack of images generated by the build-all step.  
...
## Distrib: Create the ISCE3 Distributable Image

    python -m wigwam distrib
        --base (-b) BASE_TAG
...
**--tag:** The tag of the image to be generated. Will not be prefixed with "wigwam-". this is the ISCE3 distributable image. Defaults to "isce3".

## Example Build Processes

### Typical Build Process

Many of the optional build arguments are not necessary to complete the ISCE3 distributable build. For most typical use cases, the following minimal path is all that is necessary to acquire an ISCE3 distributable image with WIGWAM. To build ISCE3 from a repository stored on the local machine, the following steps are necessary:

    python -m wigwam setup all
    python -m wigwam build-all -b setup-mamba-dev --copy-path {path/to/isce3_repo}
    python -m wigwam distrib -b setup-mamba-runtime -s build-installed

### Build From Online ISCE3 Repository

The build-all step can be modified in a small way to build from an online repository instead of a local one:

    python -m wigwam build-all -b setup-mamba-dev --repo {user}/{repo_name}
...
## Data Search: Searching The Workflow Data File
...
The user has a workflowdata file called "testing\_workflowdata.json" held in a file at "./temp/testing". To search using this workflowdata file, they will input the following:
    ...
    python -m wigwam data search -t [tags] -f temp/testing/testing_workflowdata.json
...
### Example: Fetching data for tests

As with other processes, many optional arguments in the `wigwam data fetch` command are not always necessary for typical use. Let's have an example test called `example_test` that runs the `rslc` workflow, and one called `example_test_2` that runs under the `insar` workflow:

    python -m wigwam data fetch --tests rslc:example_test insar:example_test_2

### Example: Fetching data by tag

The arguments that are shared between `wigwam data search` and `wigwam data fetch` can also be used in the same way; Any test returned by `search` will also be cached by `fetch` so long as the input arguments are the same. Thus, to acquire tagged data, the user need only use the `--tags` argument. As with the [Using multiple --tags example,](#example-using-a-non-default-workflowdata-file) imagine a user that wants all data repositories tagged with `insar` and `sanjoaquin` as well as all those tagged by `nisar` and `rslc`:

    python -m wigwam data fetch -t insar sanjoaquin -t nisar rslc
...
**--image:** The tag of the image on which to run the tests. Defaults to "isce3".  
...
### Example: Running a typical workflow

The simple case for running a workflow requires only a handful of arguments. Assume that the user wants to run an `rslc` workflow test called `example_test` on the default distributable image:

    python -m wigwam workflow rslc example_test
...
## Drop-In Sessions
    ...
    python -m wigwam dropin IMAGE_TAG
...
## Image Removal Tool
    ...
    python -m wigwam remove {IMAGE_TAG_OR_WILDCARD, ...}
        --force (-f)
        --quiet (-q)
        --ignore-prefix [!]
...
**--base:** The tag of the ISCE3 image which has a repository copied to it and in which the work directory is set to that copy's root directory, as output by the clone and insert instructions.  
**--build-type:** The CMake build type; one of "Release", "Debug", "RelWithDebInfo", "MinSizeRel". Defaults to "Release".  
**--no-cuda:** If used, this option will instruct CMake to configure and compile without using CUDA bindings.
...
**--base:** The tag of the ISCE3 image in which the work directory is in an ISCE3 repository and CMake is configured for install.
...
**--base:** The tag of the ISCE3 image in which the work directory is in an ISCE3 repository and the project is built.

## Final Commands

See the [Test section above.](test-run-optional-unit-tests)

See the [Distrib section above.](#distrib-create-the-isce3-distributable-image)
